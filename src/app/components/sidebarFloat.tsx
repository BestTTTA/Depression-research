...
const StudioMenu = () => {
    ...
    const Menu: React.FC<{ isOpen: string }> = ({ isOpen }) => {
        return (
            <div className={`fixed p-3 right-0 top-0 w-[250px] h-full z-50 bg-cyan-600 ${isOpen}`}>
    ...
    return (
        <div className="fixed bottom-5 right-8">
            <Menu isOpen={open} />
            <button onClick={toggleMenu} className='flex focus:scale-95 hover:scale-95 bg-blue-500 h-12 w-12 rounded-full text-white justify-center items-center'>
                +
            </button>
        </div>
    );
